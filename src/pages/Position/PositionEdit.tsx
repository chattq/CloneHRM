...
export default function PositionEdit({
  ...
  const handleSubmit = () => {
    formValue.FlagActive = formValue.IsActive ? "1" : "0";
    position_service.update(formValue).then((resp) => {
      if (resp.Success) {
        handleClose();
        onSuccess();
        toast.success(_l("Position data được update thành công"));
      } else {
        ShowError(resp.ErrorData);
      }
    });
        ...
        <Form fluid onChange={setFormValue} formValue={formValue}>
          <Form.Group controlId="name-9">
            ...
            <Form.Control
              name="OrgID"
              style={{ width: "100%" }}
              labelKey="NNTFullName" // các cái key ở trong data để truyền vào select
              valueKey="OrgID"
          ...
          <FlagActiveView accepter={Toggle} formValue={formValue} />
        </Form>
      </>
    );
  };
